			  ...
			  <tbody>
					...
					$res = $customer->single_customer($_SESSION['CUSID']);
				  		...
				  		$query = "SELECT * FROM `tblpromopro` pr , `tblproduct` p , `tblcategory` c 
           					 WHERE pr.`PROID`=p.`PROID` AND  p.`CATEGID` = c.`CATEGID` AND p.OWNERPHONE = '$res->PHONE' ";
				  		$mydb->setQuery($query);
				    ...
				    echo '<td style="padding:0px;">
							...
							<img  title="'.$result->CATEGORIES.'" style="width:100px;height:50px;padding:0;"  src="'. web_root."admin/products/".$result->image1 . '">
							</a></td>'; 	
